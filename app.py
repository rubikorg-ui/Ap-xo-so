...
# ==============================================================================
# 3. AUTO-HUNTER (GENETIC ALGORITHM V2 - HIỂN THỊ TIẾN TRÌNH)
# ==============================================================================
...
def run_genetic_search(target_date, _cache, _kq_db, fixed_limits, min_v, use_inv, max_allowed_nums, 
    ...
    for gen in range(generations):
        ...
        # CẬP NHẬT GIAO DIỆN (ĐỂ NGƯỜI DÙNG THẤY)
        if status_text:
            msg = f"🏃 **Đang chạy thế hệ {gen+1}/{generations}**\n\n"
            msg += f"⚡ Tốc độ: Đang xử lý {population_size} mẫu/giây\n"
            msg += f"💎 Tốt nhất hiện tại: Win {current_best['wr']:.1f}% ({current_best['avg']:.1f} số)"
