...
def fast_get_top_nums(df, p_map_dict, s_map_dict, top_n, min_v, inverse, sort_by_vote=False):
    ...
    # 4. SẮP XẾP:
    if sort_by_vote:
        # Nếu là Mode 8x (hoặc yêu cầu sort by vote) -> Ưu tiên Vote -> Điểm
        if inverse: 
        ...
        # LOGIC V24 CỔ ĐIỂN -> Ưu tiên Điểm -> Vote
        if inverse: 
...
def calculate_v24_logic_only(target_date, rolling_window, _cache, _kq_db, limits_config, min_votes, score_std, score_mod, use_inverse, manual_groups=None, max_trim=None, strategy_mode="🛡️ V24 Cổ Điển"):
    ...
    # 1. XÁC ĐỊNH NGUỒN SỐ (DATA POOL)
    if is_8x_mode:
        # Nếu là Vote 8x: Chỉ tìm cột 8X/DÀN để lấy số
        ...
        if col_8x:
            ...
            s_map_dict = {col_8x: 10} # Mod cũng chỉ lấy từ 8x
        else:
            return None # Không có cột 8x thì không chạy được mode này
    else:
    ...
    # 2. XÁC ĐỊNH CỘT LỊCH SỬ (ĐỂ CHIA NHÓM 0x-9x)
    ...
    if not manual_groups:
        ...
        for d in past_dates:
            ...
            for g in groups:
                ...
                # QUAN TRỌNG: Nếu là 8x mode -> sort_by_vote=True
